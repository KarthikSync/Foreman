...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.cmd == "harness" and args.harness_cmd == "run":
        if args.provider == "live":
            # HARNESS_LIVE: real BrowserProvider with OutlookMonarchSelectorPack.
            # Requires Playwright + Edge installed. v0.1 does not auto-install.
            from openclaw.providers.browser import BrowserReadiness
            from openclaw.providers.outlook import OutlookMonarchSelectorPack

            mode = RuntimeMode.HARNESS_LIVE
            engine = build_engine(
                ...
                browser_readiness=BrowserReadiness(
                    ...
                    headless=False,  # dev default headed
                ),
            )
        else:
